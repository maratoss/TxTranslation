...
namespace Unclassified.TxEditor.ViewModels
{
    ...
    internal class MainViewModel : ViewModelBase, IViewCommandSource
    {
        ...
        public DelegateCommand UploadFlagCommand { get; private set; }
        ...
        protected override void InitializeCommands()
        {
            ...
            UploadFlagCommand = new DelegateCommand(
                () =>
                {
                    var win = new Window { Width = 500, Height = 300, WindowStartupLocation = WindowStartupLocation.CenterOwner };
                    win.Owner = MainWindow.Instance;
                    string flagsFolder = Path.Combine(Environment.CurrentDirectory, @"Resources\pics\flags");
                    if (!Directory.Exists(flagsFolder))
                    {
                        MessageBox.Show("Can't find the directory to store flags\n" + flagsFolder);
                        return;
                    }
                    win.Content = new UploadFlagToCultureView
                    {
                        DataContext =
                            new UploadFlagToCultureViewModel(new FlagLoader(flagsFolder),
                            this.LoadedCultureNames)
                    };
                    win.ShowDialog();
                });
        }
